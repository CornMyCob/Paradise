...
/obj/structure/cursed_slot_machine/proc/determine_victor(mob/living/user)
	...
	in_use = 1
	if(prob(win_prob))
		playsound(src, 'sound/goonstation/misc/klaxon.ogg', 50, 0)
...
/obj/structure/cursed_money/attack_hand(mob/living/user)
	...
	var/obj/item/dice/d20/e20/critical_fail = new(T)
...
/obj/structure/mirror/magic/pride/curse(mob/user)
	...
	var/turf/T = get_turf(user)
	T.ChangeTurf(/turf/unsimulated/floor/chasm)
...
/obj/item/kitchen/knife/envy/afterattack(atom/movable/AM, mob/user, proximity)
	. = ..()
	if(!proximity)
		return
	if(!istype(user))
	...
	if(ishuman(user))
		...
		if(ishuman(AM))
			...
			if(U.real_name != H.dna.real_name)
				U.real_name = H.dna.real_name
				U.dna.ResetUIFrom(H)
				U.update_body()
				U.update_mutations()
				U.visible_message("<span class='warning'>[user]'s appearance shifts into [H]'s!</span>", \
