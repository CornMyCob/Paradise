...
/obj/structure/necropolis_gate/proc/toggle_the_gate(mob/user, legion_damaged)
	...
	if(open)
		...
		sleep(2.5)
		playsound(T, 'sound/magic/invoke_general.ogg', 30, TRUE, frequency = 15000)
		add_overlay(door_overlay)
		open = FALSE
	else
...
/obj/structure/stone_tile/Crossed(atom/movable/AM)
	...
	switch(fall_on_cross)
		if(COLLAPSE_ON_CROSS, DESTROY_ON_CROSS)
			if((I && I.w_class >= WEIGHT_CLASS_BULKY) || (L && L.mob_size >= MOB_SIZE_HUMAN)) //too heavy! too big! aaah!
				collapse()
		if(UNIQUE_EFFECT)
			crossed_effect(AM)
