...
/proc/spawn_rivers(target_z, nodes = 4, turf_type = /turf/simulated/floor/plating/asteroid/lavaland/basalt/lava_land_surface, whitelist_area = /area/lavaland/surface/outdoors/unexplored, min_x = RANDOM_LOWER_X, min_y = RANDOM_LOWER_Y, max_x = RANDOM_UPPER_X, max_y = RANDOM_UPPER_Y)
	...
	for(var/A in river_nodes)
		...
		cur_turf.ChangeTurf(turf_type, null, 2)
		...
		while(cur_turf != target_turf)
			...
			if(!istype(new_area, whitelist_area)) //Rivers will skip ruins
				...
				var/turf/river_turf = cur_turf.ChangeTurf(turf_type, null, 2)
				river_turf.Spread_Lava(25, 11, whitelist_area)

	for(var/WP in river_nodes)
		qdel(WP)


/turf/proc/Spread_Lava(probability = 30, prob_loss = 25, whitelisted_area)
	if(probability <= 0)
		return
	...
	for(var/F in RANGE_TURFS(1, src) - src)
		...
		if(!T || (T.density && !ismineralturf(T)) || istype(T, /turf/unsimulated) || (whitelisted_area && !istype(new_area, whitelisted_area)))
			continue
	...
	for(var/F in cardinal_turfs) //cardinal turfs are always changed but don't always spread
		var/turf/T = F
		if(!istype(T, logged_turf_type) && T.ChangeTurf(type, null, 2) && prob(probability))
			T.Spread_Lava(probability - prob_loss, prob_loss, whitelisted_area)

	for(var/F in diagonal_turfs) //diagonal turfs only sometimes change, but will always spread if changed
		var/turf/T = F
		if(!istype(T, logged_turf_type) && prob(probability) && T.ChangeTurf(type, null, 2))
			T.Spread_Lava(probability - prob_loss, prob_loss, whitelisted_area)
		else if(ismineralturf(T))
			var/turf/simulated/mineral/M = T
			M.ChangeTurf(M.turf_type, null, 2)
